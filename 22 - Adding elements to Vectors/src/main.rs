...
// This code demonstrates various ways to add elements to a vector in Rust:

// Using push() to add a single element
// Using extend() to add all elements from another collection
// Using append() to move all elements from another vector (which leaves the source vector empty)